...
public class CutLetterDigitFilter extends TokenFilter {
	...
	private void addToken(Token oriToken, int termBufferOffset, int termBufferLength, byte type) {
		...
		oriToken.copyBuffer(oriToken.buffer(), termBufferOffset, termBufferLength);
		Token token = new Token(oriToken.toString(),oriToken.startOffset()+termBufferOffset,oriToken.startOffset()+termBufferOffset+termBufferLength);
